...
try {
    ...
    match ($type) {
        ...
        'users'   => handleExportUsers($pdo),
        default   => jsonResponse(0, '未知导出类型，支持: orders, coupons, tickets, users'),
    };
} catch (Throwable $e) {
...
function handleExportUsers(PDO $pdo): void {
    ...
    foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $user) {
        $rows[] = [$user['id'], $user['username'], $user['email'] ?: '-', $user['linuxdo_username'] ?: '-', $user['linuxdo_trust_level'] ?? '-', $user['created_at']];
    }
    logAudit($pdo, 'export.users', ['count' => count($rows)]);
    outputCSV('users_' . date('Ymd_His') . '.csv',
        ['用户ID', '用户名', '邮箱', 'LinuxDO用户名', '信任等级', '注册时间'], $rows);
}
