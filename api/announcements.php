...
try {
    match ($action) {
        'list'           => handleAnnounceList($pdo),
        'detail'         => handleAnnounceDetail($pdo),
        'all'            => handleAnnounceAll($pdo),
        'add'            => handleAnnounceAdd($pdo),
        'edit'           => handleAnnounceEdit($pdo),
        'delete'         => handleAnnounceDelete($pdo),
        'toggle_top'     => handleAnnounceToggleTop($pdo),
        'toggle_status'  => handleAnnounceToggleStatus($pdo),
...
function handleAnnounceList(PDO $pdo): void {
...
function handleAnnounceDetail(PDO $pdo): void {
...
function handleAnnounceAll(PDO $pdo): void {
    checkAdmin($pdo);
...
function handleAnnounceEdit(PDO $pdo): void {
    checkAdmin($pdo);
    $id = validateInt(requestValue('id', null), 1);
    if (!$id) {
        jsonResponse(0, '公告ID无效');
...
function handleAnnounceDelete(PDO $pdo): void {
    checkAdmin($pdo);
    $id = validateInt(requestValue('id', null), 1);
    if (!$id) {
        jsonResponse(0, '公告ID无效');
...
function handleAnnounceToggleTop(PDO $pdo): void {
    checkAdmin($pdo);
    $id = validateInt(requestValue('id', null), 1);
    if (!$id) {
        jsonResponse(0, '公告ID无效');
...
function handleAnnounceToggleStatus(PDO $pdo): void {
    checkAdmin($pdo);
    $id = validateInt(requestValue('id', null), 1);
    if (!$id) {
        jsonResponse(0, '公告ID无效');
