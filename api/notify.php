...
function markPaymentRequestPaid(PDO $pdo, string $externalOrderNo, string $tradeNo, string $notifyId = ''): void {
    try {
        commerceEnsurePaymentRequestTable($pdo);
        $sql = 'UPDATE payment_requests SET status = 1, trade_no = ?, paid_at = NOW()';
        $params = [$tradeNo];
        if ($notifyId !== '') {
            // 检查 notify_id 列是否存在
            try {
                $check = $pdo->query("SHOW COLUMNS FROM payment_requests LIKE 'notify_id'");
...
try {
    ...
    // 幂等性保护：检查是否已处理过该 notify_id（防止重复回调）
    $notifyId = (string)($params['notify_id'] ?? '');
    if ($notifyId !== '') {
        $idemStmt = $pdo->prepare('SELECT id FROM payment_requests WHERE notify_id = ? AND status = 1 LIMIT 1');
        $idemStmt->execute([$notifyId]);
        if ($idemStmt->fetch()) {
            $pdo->commit();
            echo 'success';
            exit;
    ...
    if (!$order) {
        markPaymentRequestPaid($pdo, $externalOrderNo, $tradeNo, $notifyId);
        $pdo->commit();
        echo 'success';
        exit;
    }
    if ((int)$order['status'] === 1 || (int)$order['status'] !== 0) {
        markPaymentRequestPaid($pdo, $externalOrderNo, $tradeNo ?: (string)($order['trade_no'] ?? ''), $notifyId);
        $pdo->commit();
        echo 'success';
        exit;
    }
