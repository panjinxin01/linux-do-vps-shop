...
try {
    match ($action) {
        'summary'            => handleCreditSummary($pdo),
        'my_transactions'    => handleCreditMyTransactions($pdo),
        'admin_users'        => handleCreditAdminUsers($pdo),
        'admin_transactions' => handleCreditAdminTransactions($pdo),
        'admin_adjust'       => handleCreditAdminAdjust($pdo),
...
function handleCreditSummary(PDO $pdo): void {
    checkUser();
...
function handleCreditMyTransactions(PDO $pdo): void {
    checkUser();
    $pg = paginateParams();
    if (!commerceTableExists($pdo, 'credit_transactions')) {
...
function handleCreditAdminUsers(PDO $pdo): void {
    checkAdmin($pdo);
...
function handleCreditAdminTransactions(PDO $pdo): void {
    checkAdmin($pdo);
    if (!commerceTableExists($pdo, 'credit_transactions')) {
        jsonResponse(1, 'ok', []);
    }
...
function handleCreditAdminAdjust(PDO $pdo): void {
    checkAdmin($pdo);
