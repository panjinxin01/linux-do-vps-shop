...
function loadMyTickets() {
  ...
  apiFetch("api/tickets.php?action=my").then((r) => r.json()).then((data) => {
    ...
    const pMap = ["低", "中", "高", "紧急"];
    const catMap = { refund_request: "退款申请", technical: "技术支持", billing: "账单问题", other: "其他" };
    c.innerHTML = `<div class="ticket-list">${data.data.map((t) => {
      const sc = t.status == 0 ? "wait" : t.status == 1 ? "on" : "off";
      const st = t.status == 0 ? "待回复" : t.status == 1 ? "已回复" : "已关闭";
      const cat = catMap[t.category] || t.category || "其他";
      const pri = pMap[parseInt(t.priority || 1)] || "中";
      return `<div class="ticket-item" onclick="showTicketDetail(${t.id})">
        <div class="ticket-header">
          <span class="ticket-title">${escapeHtml(t.title)}</span>
          <span class="badge ${sc}">${st}</span>
        </div>
        <div class="ticket-meta">
          <span><svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align:-1px;margin-right:3px"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>${escapeHtml(t.updated_at || "")}</span>
          <span>${escapeHtml(cat)}</span>
          <span>优先级：${escapeHtml(pri)}</span>
          ${t.order_no ? `<span>订单：${escapeHtml(t.order_no)}</span>` : ""}
        </div>
      </div>`;
    }).join("")}</div>`;
  });
...
function showTicketDetail(id) {
  Promise.all([
    ...
    // 头部信息
    const headerHtml = `<div class="ticket-detail-header">
      <div class="ticket-detail-status-row">
        <span class="badge ${sc}">${st}</span>
        ${t.order_no ? `<span style="font-size:12px;color:var(--text-muted)">关联订单：<code style="color:var(--primary)">${escapeHtml(t.order_no)}</code></span>` : ""}
      </div>
      <div class="ticket-detail-meta">
        <div class="ticket-detail-meta-item">
          <svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align:-1px;margin-right:3px"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
          分类 <span>${escapeHtml(catMap[t.category] || t.category || "其他")}</span>
        </div>
        <div class="ticket-detail-meta-item">
          <svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align:-1px;margin-right:3px"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>
          优先级 <span>${escapeHtml(pMap[parseInt(t.priority || 1)] || "中")}</span>
        </div>
        <div class="ticket-detail-meta-item">
          <svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align:-1px;margin-right:3px"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
          更新 <span>${escapeHtml(t.updated_at || "-")}</span>
        </div>
      </div>
    </div>`;

    // 回复列表
    const rHtml = (t.replies || []).map((r) => `<div class="ticket-reply ${r.user_id ? "user" : "admin"}">
    ...
    // 时间线
    const evHtml = (t.events || []).length ? `<div class="ticket-timeline"><div class="ticket-timeline-title"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align:-2px;margin-right:4px"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>处理时间线</div>${t.events.map((e) => `<div class="ticket-timeline-item"><div class="ticket-timeline-dot"></div><div class="ticket-timeline-content"><strong>${escapeHtml(e.event_type || "event")}</strong><span>${escapeHtml(e.content || "")}</span><time>${escapeHtml(e.created_at || "")}</time></div></div>`).join("")}</div>` : "";
    ...
    if (t.status != 2) {
      ...
      composeHtml = `<div class="ticket-reply-compose">
        ...
        <div class="ticket-reply-compose-actions">
          <input type="file" id="ticketFile" accept="${fileAccept}">
          <button class="btn btn-outline" style="padding:6px 12px;font-size:12px" onclick="uploadUserTicketAttachment(${t.id})">
    ...
    document.getElementById("ticketDetailBody").innerHTML = headerHtml +
      `<div class="ticket-replies">${rHtml}</div>` + evHtml + atHtml + composeHtml;

    // 底部按钮
    if (t.status != 2) {
      document.getElementById("ticketDetailFoot").innerHTML = `<div class="ticket-detail-actions">
        <button class="btn btn-outline" onclick="closeTicket(${t.id})">
          <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align:-2px;margin-right:4px"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>关闭工单
        </button>
        <button class="btn btn-primary" onclick="replyTicket(${t.id})">
          <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align:-2px;margin-right:4px"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>发送回复
        </button>
      </div>`;
    } else {
      document.getElementById("ticketDetailFoot").innerHTML = `<button class="btn btn-primary" style="width:100%" onclick="closeTicketDetail()">关闭</button>`;
    }
