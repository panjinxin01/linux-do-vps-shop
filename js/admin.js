  ...
  Promise.all([
    ...
    const headerHtml = `<div class="ticket-detail-header">
      <div class="ticket-detail-status-row">
        ...
        ${oi ? `<span class="badge ${String(oi.delivery_status || "") === "refunded" ? "off" : "on"}">
          ${escapeHtml(["待支付","已支付","已退款","已取消"][parseInt(oi.status || 0)] || "-")} / ${escapeHtml(oi.delivery_status || "-")}
        </span>` : ""}
      </div>
      <div class="ticket-detail-meta">
        <div class="ticket-detail-meta-item">用户 <span>${escapeHtml(t.username || "-")}</span></div>
        ${t.order_no ? `<div class="ticket-detail-meta-item">订单 <span>${escapeHtml(t.order_no)}</span></div>` : ""}
        <div class="ticket-detail-meta-item">分类 <span>${escapeHtml(t.category || "other")}</span></div>
        <div class="ticket-detail-meta-item">更新 <span>${escapeHtml(t.updated_at || "-")}</span></div>
      </div>
    </div>`;
    ...
    if (t.category === "refund_request") {
      refundMeta = `<div class="refund-admin-card">
        <div class="refund-admin-card-header">
          <div>
            <div class="refund-admin-card-title">
      ...
      if (t.status != 2) {
        refundMeta += `<div class="refund-approve-section">
          <div class="refund-admin-grid">
    ...
    if (t.status != 2) {
      fh = `<div class="admin-inline-actions">
        <button class="btn btn-outline" onclick="adminCloseTicket(${t.id})">
        ...
        <button class="btn btn-primary" onclick="adminReplyTicket(${t.id})">
        ...
        ${t.category === "refund_request" ? `<button class="btn btn-danger" onclick="approveRefundTicket(${t.id})">
          <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align:-2px;margin-right:4px"><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/></svg>同意退款
        </button>` : ""}
      </div>`;
    }
