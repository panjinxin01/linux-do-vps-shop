...
function ldcpay_format_private_key_pem(string $privateKey): ?string {
    ...
    // 结构: SEQUENCE { INTEGER 0, SEQUENCE { OID }, OCTET STRING { OCTET STRING { seed } } }
 ...
 *
 * Ed25519 SubjectPublicKeyInfo 需要包含算法 OID，不能直接封装 raw key。
 * 这里构造符合 RFC 8410 的标准 DER 编码。
 */
function ldcpay_format_public_key_pem(string $publicKey): ?string {
    ...
    // 构造 SubjectPublicKeyInfo DER (Ed25519 OID: 1.3.101.112)
